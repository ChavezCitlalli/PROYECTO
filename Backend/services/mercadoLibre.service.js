...
async function buscarProductos( palabra ) {
    try {
        let resp = await fetch('https://api.mercadolibre.com/sites/MLM/search?category=MLM1000&q='+ palabra);        
        let data = await resp.json();
        let result = data.results;
        if (result.length == 0) {
...
async function getCategorias() {
    try {
        let resultado = await fetch ('https://api.mercadolibre.com/categories/MLM1000');
...
module.exports = { Producto, Categoria, mandarProductos, mandarBusqueda, mandarCategorias, mandarProductosXcategoria };